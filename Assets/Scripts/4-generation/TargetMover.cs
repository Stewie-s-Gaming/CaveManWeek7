...
public class TargetMover: MonoBehaviour {
    ...
    private void MakeOneStepTowardsTheTarget() {
        ...
        if (shortestPath.Count >= 2) {
            if (allowedTiles.ContainSlow(tilemap.GetTile(shortestPath[1])) && originalTime == timeBetweenSteps)
            {
                timeBetweenSteps *= 2;
            }
            else if (!allowedTiles.ContainSlow(tilemap.GetTile(shortestPath[1])) && originalTime != timeBetweenSteps) {
                timeBetweenSteps = originalTime;
            }
